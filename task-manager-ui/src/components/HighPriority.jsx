...
export default function HighPriority({ token }) {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const highPriorityTasks = data?.data?.filter(task => task.priority === "High") || [];
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-semibold text-red-600">High Priority Tasks</h2>

      {tasks.length === 0 ? (
        <p className="text-gray-600 mt-4">No high-priority tasks available.</p>
      ) : (
      ...
      <button className="mt-4 bg-red-500 text-white px-4 py-2 rounded-md" onClick={() => navigate("/dashboard")}>
        Back to Dashboard
      </button>
    </div>
  );
